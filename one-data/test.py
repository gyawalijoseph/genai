...
class DynamicMermaidERGenerator:
    ...
    def _generate_database_section(self, db: Dict) -> List[str]:
        """Generate database entity and one consolidated table"""
        lines = []
        ...
        # Create one consolidated table with all columns from all entities
        all_columns = []
        ...
        if all_columns:
            table_id = f"{db_id}_TABLES"
            lines.append(f"    {table_id} {{")
            lines.append(f'        string consolidated_tables "All tables in {db["name"]}"')
    ...
    def _generate_relationships(self, databases: List[Dict]) -> List[str]:
        ...
        for db in databases:
            ...
            # Check if database has any columns across all entities
            has_columns = any(entity.get('columns', []) for entity in db['entities'])
